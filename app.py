...
if 'METINLER' not in st.session_state:
    st.session_state.METINLER = [
        ...
        },
        {
            ...
            "icerik": "Küresel ısınma, kutup buzullarının erimesine ve deniz seviyelerinin yükselmesine neden olmaktadır. Bilim insanları, karbon emisyonunun azaltılmaması durumunda 2050 yılına kadar ekosistemde geri dönülemez hasarlar oluşacağını öngördektedir...", 
        ...
        },
        {
            ...
            "icerik": "Dijital paralar ve yapay zeka tabanlı ticaret sistemleri, klasik bankacılık anlayışını kökten değiştiriyor. 2026 yılında küresel ekonominin %30'unun blokzincir altyapısına geçmesi bekleniyor...", 
        ...
        },
        {
            ...
            "icerik": "Hibrit eğitim modelleri ve kişiselleştirilmiş öğrenme algoritmaları, sınıf içi eğitimi daha verimli hale getiriyor. Öğrenciler artık kendi hızlarında öğrenirken, öğretmenler rehber rolünü üstleniyor...", 
...
# --- EKRAN 1: GİRİŞ (Ad, Soyad ve Sınıf) ---
if st.session_state.step == "GIRIS":
    st.title("Araştırma Veri Toplama Paneli - Zek Hamamcı :)))")
    st.write("Lütfen bilgilerinizi eksiksiz girerek teste başlayınız.")
    
    ad = st.text_input("Adınız Soyadınız:", placeholder="Örn: Ahmet Yılmaz")
    sinif = st.text_input("Sınıfınız:", placeholder="Örn: 10-C, 12-A vb.")
    ...
    if st.button("Sisteme Giriş Yap"):
        if ad and sinif:
            ...
            st.warning("Lütfen hem isminizi hem de sınıfınızı belirtiniz.")
    ...
    if not st.session_state.warning_seen:
        st.markdown("""
            <div class="warning-box">
                <h2 style="color: #856404; margin-top:0;">⚠️ DİKKAT: Yeni Metne Geçildi!</h2>
                <p style="color: #856404; font-size: 18px;">Soruları cevaplamadan önce lütfen <b>en üstteki yeni metni</b> okuyun. Hazır olduğunuzda onaylayın.</p>
            </div>
        """, unsafe_allow_html=True)
                    ...
                    if p1 and p2 and p3:
                        ...
                        with st.spinner("Veriler kaydediliyor..."):
                            try:
                                ...
                                # VERİ SATIRI OLUŞTURMA (Sınıf dahil)
                                yeni = {
...
# --- EKRAN 3: BİTİŞ ---
    ...
    st.balloons()
    st.success(f"Sayın {st.session_state.user_name}, katılımınız için teşekkürler!")
