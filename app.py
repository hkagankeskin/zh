...
# Sizin verdiğiniz güncel sorular
SORULAR = [
...
# --- EKRAN 2: TEST SÜRECİ ---
    ...
    # KESİN ÇÖZÜM: Tüm içeriği dinamik key'li bir container içine alıyoruz.
    # Bu sayfa her değiştiğinde (idx değiştikçe) Streamlit sayfayı EN ÜSTTEN başlatır.
    with st.container(key=f"content_block_{idx}"):
        ...
        if idx < 3:
            if st.button("Sonraki Metne Geç ➔", key=f"btn_next_{idx}"):
                if p1 is None or p2 is None or p3 is None:
                    ...
                    st.rerun() # Key değiştiği için sayfa en üste fırlayacaktır.
        else:
            if st.button("Testi Tamamla ve Verileri Kaydet", key="btn_finish"):
                if p1 is None or p2 is None or p3 is None:
