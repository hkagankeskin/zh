...
st.markdown("""
    ...
    .active-tab { background: #ffffff; height: 34px; padding: 0 20px; border-radius: 8px 8px 0 0; display: flex; align-items: center; font-size: 12px; font-family: sans-serif; color: #3c4043; margin-top: 8px; }
...
# --- EKRAN 2: TEST SÜRECİ ---
    ...
    # Sayfanın en başına görünmez bir çapa koyuyoruz
    ...
    st.caption("1 = Hiç katılmıyorum, 6 = Tamamen katılıyorum")
    ...
    # NAVİGASYON: Sayfayı en üste atan asıl mekanizma
    if idx < 3:
        if st.button("Sonraki Metne Geç ➔"):
            if p1 is None or p2 is None or p3 is None:
                ...
                # ÖNEMLİ: URL'ye sayfa numarası ekleyerek tarayıcıyı scroll resetlemeye zorlar
                st.query_params["p"] = str(st.session_state.current_text)
                st.rerun()
    else:
            ...
            if p1 is None or p2 is None or p3 is None:
                ...
                with st.spinner("Kaydediliyor..."):
                    try:
                        ...
                        yeni_satir = {"tarih": datetime.now().strftime("%d/%m/%Y %H:%M"), "ad_soyad": st.session_state.user_name}
...
# --- EKRAN 3: BİTİŞ ---
    ...
    st.balloons()
    st.success("Tebrikler! Yanıtlarınız başarıyla kaydedildi.")
